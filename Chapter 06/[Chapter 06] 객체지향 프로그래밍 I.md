...
- `for`문 또는 `while`문이 블럭 내에 선언된 지역 변수는, 지역 변수가 선언된 블럭 내에서만 사용 가능하며, 블럭을 벗어나면 소멸되어 사용할 수 없게 된다.
...
2. 중복된 코드의 제거
    ...
    프로그램을 작성하다보면, 같은 내용의 문장들이 여러 곳에 반복해서 나타나는데, 이렇게 반복되는 문장들을 묶어서 하나의 메서드로 작성해 놓으면, 반복되는 문장들 대신 메서드를 호출하는 한 문장으로 대체할 수 있다.
    
    반복적으로 나타나는 문장들을 메서드로 만들어 사용하면, 코드의 중복이 제거되고, 변경 사항이 발생했을 때 해당 메서드만 수정하면 되므로 관리도 쉽고 오류의 발생 가능성도 낮아진다.
    
3. 프로그램의 구조화
    
    큰 규모의 프로그램에서는 문장들을 작업 단위로 나눠 여러 개의 메서드를 담아 프로그램의 구조를 단순화 시키는 것이 필수적이다. main 메서드는 프로그램의 전체 흐름이 한 눈에 들어올 정도로 단순하게 구조화하는 것이 좋다.
...
MyMath 클래스의 add 메서드를 호출하기 위해서는 먼저 `MyMath mm = new MyMath();` 와 같이 클래스의 인스턴스를 생성한 다음 참조변수 mm을 통해야 한다.

메서드가 호출되면 지금까지 실행중이던 메서드는 실행을 잠시 멈추고 호출된 메서드의 문장들이 실행된다. 호출된 메서드의 작업이 모두 끝나면, 다시 호출한 메서드로 돌아와 이후의 문장들을 실행한다.

> `static` 메서드가 같은 클래스 내의 인스턴스 메서드를 호출할 수 없는 이유?
> 
- `static` 메서드는 클래스가 로딩될 때 메서드 영역에 올라가며, 인스턴스 생성 없이도 사용할 수 있다.
- 반면, 인스턴스 메서드는 객체가 생성되어야만 사용할 수 있다.
- 따라서, **정적 메서드는 객체가 생성되기 전에 존재하고, 인스턴스 메서드는 객체가 생성된 후에 사용될 수 있으므로, 두 메모리 구조와 실행 시점이 달라 `static` 메서드가 인스턴스 메서드를 직접 호출하는 것은 불가능하다.**
- 아래와 같이 인스턴스를 내부에서 생성하면 호출 가능하다.
    
    ```java
    static void staticMethod() {
        Example e = new Example();
        e.instanceMethod();  // ✅ 인스턴스 생성 후 호출은 가능
    }
    ```
    ...
    public class MyClass {
      public static void main(String args[]) {
        /**
         * MyClass.java:17: error: non-static method print() cannot be referenced from a static context
            print();  
            ^
            1 error
        **/
